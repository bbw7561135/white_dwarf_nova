...
ComputationalCell SafeLinearGauss::Interp(ComputationalCell const& cell, size_t cell_index, Vector2D const& cm,
...
vector<pair<ComputationalCell, ComputationalCell> > SafeLinearGauss::operator() 
	...
	for (size_t i = 0; i < edge_number; ++i)
	{
		...
		else
		{
		  ComputationalCell const& cell = safe_retrieve(ghost_cells,static_cast<size_t>(edge.neighbors.first));
			cell_temp.first = interp
			  (cell, ghost_.GetGhostGradient(tess,cells,rslopes_,static_cast<size_t>
		...
		else
		{
		  const ComputationalCell& cell = safe_retrieve
			...
			cell_temp.second = interp(cell, ghost_.GetGhostGradient
						  (tess, cells, rslopes_, static_cast<size_t>
...
vector<pair<ComputationalCell, ComputationalCell> >& SafeLinearGauss::GetSlopes(void)const
{
	return rslopes_;
}

vector<pair<ComputationalCell, ComputationalCell> >& SafeLinearGauss::GetSlopesUnlimited(void)const
{
